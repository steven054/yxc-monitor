...
class EnterpriseWeChatSender:
    def __init__(self):
        # 企业微信机器人webhook地址
        self.webhook_url = os.getenv('ENTERPRISE_WECHAT_WEBHOOK_URL', '')
...
def main():
    ...
    print()
    print("📱 设置步骤：")
    print("1. 创建企业微信群（或使用现有企业微信群）")
    print("2. 在企业微信群中添加机器人")
    print("3. 获取机器人的webhook地址")
    print("4. 将webhook地址配置到.env文件中")
    print("5. 邀请需要接收消息的微信用户加入企业微信群")
    print()
    
    # 检查配置
    webhook_url = os.getenv('ENTERPRISE_WECHAT_WEBHOOK_URL', '')
    if not webhook_url:
        print("❌ 请在.env文件中配置ENTERPRISE_WECHAT_WEBHOOK_URL")
        print("\n🔧 配置示例：")
        print("ENTERPRISE_WECHAT_WEBHOOK_URL=https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=YOUR_KEY")
        return
    
    sender = EnterpriseWeChatSender()
    
    print("📤 自动发送监控报告到企业微信群...")
    success = sender.send_comprehensive_report()
    
    if success:
        print("\n🎉 企业微信群自动发送成功！")
        print("📱 请检查企业微信群，应该能看到统计信息和表格图片")
    else:
        print("\n❌ 企业微信群自动发送失败")
